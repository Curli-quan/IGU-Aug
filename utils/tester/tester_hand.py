...
class Tester(object):
    ...
    def test(self, model, epoch=0, rank=-1, collecting_all_mre=False, draw=False):
        ...
        for data in tqdm(self.dataloader, ncols=70):
            ...
            pred_landmark = rearrange(pred_landmark, "c n -> n c")
            # self.evaluater.record_hand_old(pred_landmark, torch.Tensor(landmark_list).detach().cpu().numpy())

            names.append(data['name'][0])
            if draw:
                pred_landmark = pred_landmark[:, ::-1]
        ...
        d = self.evaluater.cal_metrics(collecting_all_mre=collecting_all_mre)
        return {**d, "name":names}

    def draw(self, model):
        self.test(model, draw=True)
