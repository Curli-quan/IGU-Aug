...
class Tester(object):
    def __init__(self, logger, config, gender, split="Test", args=None,
        ...
        self.split = split
        dataset_1 = HandXray(config['dataset']['pth'], gender=gender, split=split, num_repeat=1)
    ...
    def test_func(self, model, epoch=None, rank='cuda', oneshot_id=21, dump_label=False, draw=False, savedir_pred=None):
        net = model
        net.eval()
        config = self.config

        # oneshot_id = oneshot_id if oneshot_id is not None else self.default_oneshot_id
        print(f'tester_hand_ssl. ID Oneshot : {oneshot_id}')
        ...
        for i, data in enumerate(self.dataloader_1):
            ...
            if savedir_pred is not None:
                preds_collect.append(preds)
                gt_collect.append(landmark_list)
                img_collect.append(img[0][0].detach().cpu().numpy())
        ...
        if savedir_pred is not None:
            collect = {"pred": preds_collect, "gt":gt_collect, "img":img_collect}
            np.save(os.path.join(savedir_pred, f"handpred_oneshotid{oneshot_id}.npy"), collect)
