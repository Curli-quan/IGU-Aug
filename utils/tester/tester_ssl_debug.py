...
class Tester(object):
    ...
    def draw_one_cosmap(self, model, rank='cuda', oneshot_id=-1):
        net = model
        ...
        for ID, data in enumerate(self.dataloader_1):
            ...
            for id_mark in range(one_shot_feature.shape[0]):
                ...
                save_image(final_cos, tfilename(f"paper_mi/std/final_cos_{id_mark}.jpg"))
            break


    def test_func(self, model, epoch=None, rank='cuda', oneshot_id=-1, dump_label=False, draw=False, collect_details=False, savedir_pred=None):
        net = model
        # if net is not None:
        #     self.model = net
        ...
        if savedir_pred is not None:
            np.save(os.path.join(savedir_pred, f"preds_oneshotid{oneshot_id}.npy"), preds_collect)
