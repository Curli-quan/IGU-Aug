...
def test(logger, config):
    ...
    # oneshot_id = 0
    # learner.eval()
    # res_dict = tester.test(learner, epoch=0, rank="cuda", oneshot_id=oneshot_id, draw=False, savedir_pred="/home1/quanquan/code/landmark/code/tproj/paper_scp/predictions")
    # # logger.info(f"")
    # res_dict['oneshot_id'] = oneshot_id
    # logger.info(f"Results: {res_dict}")
...
if __name__ == '__main__':
    ...
    tconfig = TConfig(mode="sc", file=__file__, parser=parser, ex_config=EX_CONFIG)
    config = tconfig.get_config()
    logger = tconfig.get_logger()
    args = tconfig.get_args(parser)
