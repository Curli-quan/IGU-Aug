...
from utils.tester.tester_hand_ssl_fair import Tester
from datasets.hand.hand_ssl_hard import HandXray
...
class Learner(LearnerWrapper):
    def __init__(self, logger, config, *args, **kwargs):
        ...
        self.logger = logger
        self.config = config
        self.net = UNet_Pretrained(3, non_local=config['training']['non_local'], emb_len=config['training']['emb_len'])
        self.net_patch = UNet_Pretrained(3, emb_len=config['training']['emb_len'])
    ...
    def load(self, path=None, **kwargs):
        if path is None:
            path = self.config['training']['pretrain_model']
...
def train(logger, config):
    tester = Tester(logger, config, gender=config['base']['gender'], split="Test", upsample="nearest")
    monitor = Monitor(key='mre', mode='dec')
    dataset_train = HandXray(
            config['dataset']['pth'],
            patch_size=config['training']['patch_size'],
            split="EASY",
            cj_brightness=config['training']['cj_brightness'],
            cj_contrast=config['training']['cj_contrast'],
            num_repeat=5)
...
def test(logger, config):
    ...
    oneshot_id = 21
    res_dict = tester.test(learner, oneshot_id=oneshot_id, draw=False, savedir_pred="/home1/quanquan/code/landmark/code/tproj/paper_scp/predictions")
...
def test_multi(logger, config, *args, **kwargs):
    ...
    for i in range(0, 200):
        ids = np.random.choice(np.arange(1, 150), config['training']['idnum'], replace=False)
