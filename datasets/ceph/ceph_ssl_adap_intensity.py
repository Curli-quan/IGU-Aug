...
from torchvision.transforms import ColorJitter as CJ_std
from typing import Tuple, List, Optional
from torch import Tensor
from torchvision.transforms import functional as F


class ColorJitter(CJ_std):
    @staticmethod
    def get_params(
    ...
    def _entr_to_params(self, entr, mapping_rule):
        if mapping_rule =="linear":
            b_min = 0.6 + 0.1*(entr-3) # 0.6-0.8 ,3, 5
            b_max = 1.6 - 0.2*(entr-3) # 1.6-1.2, 3, 5
            c_min = 0.5 + 0.15*(entr-3) # 0.5-0.8
            c_max = 1.7 - 0.2*(entr-3) # 1.7-1.3
            return [np.clip(b_min, 0, 1), np.clip(b_max, 1, 999)], [np.clip(c_min, 0, 1), np.clip(c_max, 1, 999)]
        elif mapping_rule == "exp":
            gamma=0.5
            b_min = 0.092 + 0.4*(entr)**gamma # 0.6-0.8 ,3, 5
            b_max = 2.984 - 0.8*(entr)**gamma # 1.6-1.2, 3, 5
            c_min = -0.5 + 0.6*(entr)**gamma # 0.5-0.8
            c_max = 3.08 - 0.8*(entr)**gamma # 1.7-1.3
            # print("debug_entr_to_params", [np.clip(b_min, 0, 1), np.clip(b_max, 1, 999)], [np.clip(c_min, 0, 1), np.clip(c_max, 1, 999)])
            return [np.clip(b_min, 0, 1), np.clip(b_max, 1, 999)], [np.clip(c_min, 0, 1), np.clip(c_max, 1, 999)]
        else:
            raise NotImplementedError

    def forward(self, img, entr, mapping_rule):
        brightness, contrast = self._entr_to_params(entr, mapping_rule)
...
class Cephalometric(data.Dataset):
    def __init__(self, pathDataset, mode="Train", size=384, patch_size=192,
                 ...
                 adap_params=None, mapping_rule="linear"):
        ...
        self.aug_transform_postprocess = transforms.Compose([
            transforms.ToTensor(),
            transforms.Normalize([0], [1])
        ])
    ...
    def augment_patch_adap(self, tensor, entr_value):
        image = to_PIL(tensor)
        aug_image = self.colorjitter(image, entr_value, mapping_rule=self.mapping_rule)
        aug_image = self.aug_transform_postprocess(aug_image)
        return aug_image
