...
class Evaluater(object):
    ...
    def record_hand_old(self, pred, landmark):
        ...
        assert landmark.shape == (37, 2), f"Got {landmark.shape}, should be (37, 2)"
        scale_rate = WRIST_WIDTH / radial(landmark[0], landmark[4])
        c = len(pred)
        assert c == 37, f"Got {c}"
        diff = np.zeros([c, 2], dtype=float)  # y, x
    ...
    def cal_metrics(self, debug=False, collecting_all_mre=False):
        # calculate MRE SDR
        ...
        if debug:
            import ipdb; ipdb.set_trace()
        if collecting_all_mre: 
            ret_dict = {'mre': mre, "all_mre": temp.mean(axis=1),}
        else:
            ret_dict = {'mre': mre}
