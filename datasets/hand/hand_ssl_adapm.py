...
class HandXray(data.Dataset):
    def __init__(self, pathDataset='/home1/quanquan/hand/hand/',
                 split="Train", size=[384, 384], extra_aug_loss=False, patch_size=64, rand_psize=False,
                 retfunc=1, use_prob=False, multi_output=False, cj_brightness=0.15, cj_contrast=0.25,
                 prob_map_dir=None, entr_map_dir="/home1/quanquan/datasets/hand/hand/entr4/train", num_repeat=1, prob_mode="gaussian"):
        """
        ...
        self.ori_train_list = self.train_list
        
        if split == "HARD":
            self.filter_list = HARD_LIST   
            self.train_list = [self.train_list[i] for i in self.filter_list]        
            self.landmarks_train = [self.landmarks_train[i] for i in self.filter_list]     
        elif split == "EASY":
            self.filter_list = reverse_filter(np.arange(len(self.train_list)), HARD_LIST)
            self.train_list = [self.train_list[i] for i in self.filter_list]        
            self.landmarks_train = [self.landmarks_train[i] for i in self.filter_list]   

        self.xy = np.arange(0, self.size[0] * self.size[1])

        if split in ["Oneshot", "Train", "HARD", "EASY"]:
            self.istrain = True
        ...
        if split in ['Train', 'Oneshot', "HARD", "EASY"]:
    ...
    def prob_map_point(self, all_landmarks, size=(384,384)):
        ...
        for i, landmarks in enumerate(all_landmarks):
            ...
            if not os.path.exists(path):
                ...
                assert prob_map.shape[-2:] == tuple(size), f"use existed probmap, Got {prob_map.shape}, it should be {size}"
            self.prob_map_dict.append(prob_map)
        self.prob_map_dict = [self.prob_map_dict[i] for i in self.filter_list]       

    def entr_map_from_image(self, size=(384, 384), temperature=0.1, inverse=False):
        print("Accessing entropy maps!")
        self.entr_map_dict = []
        for i, im_path in enumerate(self.ori_train_list):
        ...
        print(f"Got entropy maps, list_len: {len(self.train_list)}")
        self.entr_map_dict = [self.entr_map_dict[i] for i in self.filter_list]        
    ...
    def _process1(self, index, img):
        ...
        entr = self.entr_map_dict[index][raw_h, raw_w]
        # print("entr", entr)
        if self.multi_output:
            ...
            return {'raw_imgs': img, 'raw_loc': torch.LongTensor([raw_h, raw_w]),
                ...
                'crop_imgs2': crop_imgs2, 'chosen_loc2': torch.LongTensor([chosen_h2, chosen_w2]), 'ID': index, "entr":entr}
        else:
            return {'raw_imgs': img, 'raw_loc': torch.LongTensor([raw_h, raw_w]),
                'crop_imgs': crop_imgs, 'chosen_loc': torch.LongTensor([chosen_h, chosen_w]), 'ID': index, "entr": entr}
