...
class HandXray(data.Dataset):
    def __init__(self,
        ...
        assert len(self.test_list) == 300, f"Got error testlist:{len(self.test_list)} != 300"
    ...
    def __getitem__(self, index):
        ...
        img, landmark_list, index, img_shape, name = self._get_data(index)
        if self.ret_mode == "all":
            d = self._process_all(img, landmark_list, index, img_shape)
            return {**d, "name":name}
        elif self.ret_mode == "heatmap_only":
            d = self._process_heatmap_only(img, landmark_list, index, img_shape)
            return {**d, "name":name}
        ...
        #     return self._process_onehot_heatmap(img, landmark_list, index, img_shape)
            ...
            return {'img':img, 'landmark_list': landmark_list, "index": index, "img_shape": img_shape, "name":name}
        else:
            raise NotImplementedError

    def _get_data(self, index, return_ori=False):
        item_path = self.loading_list[index]
        parent, name = os.path.split(item_path)
        ...
        if return_ori:
            return item, landmark_list, index, img_shape, ori_img, name
        return item, landmark_list, index, img_shape, name

    def _process_all(self, item, landmark_list, index, img_shape, name=None):
        """
