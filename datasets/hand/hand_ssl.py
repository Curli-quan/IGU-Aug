...
class TestHandXray(data.Dataset):
    def __init__(self, pathDataset='/home1/quanquan/datasets/hand/hand/jpg/', label_path='/home1/quanquan/datasets/hand/hand/all.csv',
    ...
    def get_ori_img(self, index=1, debug=False):  # some bugs in index=0
        img_path = self.train_list[index]
        print("oneshot img pth: ", img_path)
        ori_img = Image.open(img_path)
        # if self.transform is not None:
        ...
        shape = np.array(ori_img).shape[::-1]
        landmark_list = self.resize_landmark(self.landmarks_train[index]['landmark'], shape)
        return ori_img, landmark_list, shape  # disacrd the index
